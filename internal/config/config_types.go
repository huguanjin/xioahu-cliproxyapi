...
// ProxyPoolEntry represents a named outbound proxy that can be assigned to auth accounts.
// Binding an account to an entry copies its ProxyURL onto the account's own proxy-url field;
// no runtime link is kept, so editing or deleting an entry never affects already-bound accounts.
type ProxyPoolEntry struct {
	// ID uniquely identifies this entry; generated automatically when omitted.
	ID string `yaml:"id" json:"id"`

	// Name is a human-readable label shown when selecting this entry.
	Name string `yaml:"name" json:"name"`

	// ProxyURL is the outbound proxy address, e.g. socks5://user:pass@host:port.
	ProxyURL string `yaml:"proxy-url" json:"proxy-url"`

	// Account optionally records the proxy provider's account/login for reference.
	Account string `yaml:"account,omitempty" json:"account,omitempty"`

	// ExitIP optionally records the proxy's known exit IP for reference.
	ExitIP string `yaml:"exit-ip,omitempty" json:"exit-ip,omitempty"`

	// Notes holds free-form operator notes about this entry.
