...
// SanitizeProxyPool trims fields, drops entries without a proxy-url, assigns an ID to
// entries missing one, and deduplicates by ID (last one wins).
func (cfg *Config) SanitizeProxyPool() {
	if cfg == nil {
		return
	}
	entries := cfg.ProxyPool
	...
	for i := range entries {
		...
		entry.Account = strings.TrimSpace(entry.Account)
		entry.ExitIP = strings.TrimSpace(entry.ExitIP)
		entry.Notes = strings.TrimSpace(entry.Notes)
		if idx, exists := seen[entry.ID]; exists {
			out[idx] = entry
			continue
		}
