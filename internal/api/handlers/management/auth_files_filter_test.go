...
func TestListAuthFilesFiltersByHasProxy(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "")

	authDir := t.TempDir()
	boundPath := filepath.Join(authDir, "bound.json")
	unboundPath := filepath.Join(authDir, "unbound.json")
	if errWrite := os.WriteFile(boundPath, []byte(`{"type":"codex"}`), 0o600); errWrite != nil {
		t.Fatalf("failed to write auth file: %v", errWrite)
	}
	if errWrite := os.WriteFile(unboundPath, []byte(`{"type":"codex"}`), 0o600); errWrite != nil {
		t.Fatalf("failed to write auth file: %v", errWrite)
	}
	...
	registerAuthForLookupTest(t, manager, &coreauth.Auth{
		...
		Attributes: map[string]string{"path": boundPath},
	})
	registerAuthForLookupTest(t, manager, &coreauth.Auth{
		ID:         "unbound",
		...
		Provider:   "codex",
		Status:     coreauth.StatusActive,
		Attributes: map[string]string{"path": unboundPath},
	})
	...
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/v0/management/auth-files?has_proxy=false", nil)
	h.ListAuthFiles(ctx)
	...
	if len(payload.Files) != 1 || payload.Files[0]["id"] != "unbound" {
		t.Fatalf("has_proxy=false files = %#v, want only unbound", payload.Files)
	}
	if got, ok := payload.Files[0]["has_proxy"].(bool); !ok || got {
		t.Fatalf("has_proxy field = %#v, want false", payload.Files[0]["has_proxy"])
	}

	rec = httptest.NewRecorder()
	ctx, _ = gin.CreateTestContext(rec)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/v0/management/auth-files?has_proxy=true", nil)
	h.ListAuthFiles(ctx)
	...
	if len(payload.Files) != 1 || payload.Files[0]["id"] != "bound" {
		t.Fatalf("has_proxy=true files = %#v, want only bound", payload.Files)
	}
	if got, _ := payload.Files[0]["proxy_url"].(string); got != "socks5://1.2.3.4:1080" {
		t.Fatalf("proxy_url = %#v, want socks5://1.2.3.4:1080", payload.Files[0]["proxy_url"])
	}
}
