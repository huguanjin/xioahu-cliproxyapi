...
func (h *Handler) ListAuthFiles(c *gin.Context) {
	...
	hasProxyFilter := strings.TrimSpace(c.Query("has_proxy"))
	...
	for _, auth := range auths {
		...
		if hasProxyFilter != "" {
			wantProxy, errParse := strconv.ParseBool(hasProxyFilter)
			if errParse == nil && (strings.TrimSpace(auth.ProxyURL) != "") != wantProxy {
				continue
			}
		}
	...
	for _, e := range entries {
		...
		if info, errInfo := e.Info(); errInfo == nil {
			...
			if data, errRead := os.ReadFile(full); errRead == nil {
				...
				fileData["has_proxy"] = strings.TrimSpace(gjson.GetBytes(data, "proxy_url").String()) != ""
...
func (h *Handler) buildAuthFileEntryLocked(auth *coreauth.Auth) gin.H {
	...
	entry["has_proxy"] = strings.TrimSpace(auth.ProxyURL) != ""
